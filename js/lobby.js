...
const railItems = [
  ...
  {l:'Slots',        t:'sec-slots',     cat:'slots',     ic:'<img src="images/slots-777.png.png" style="width:26px;height:26px;object-fit:contain;filter:drop-shadow(0 1px 3px rgba(0,0,0,.4))">'},
      ...
      ${row.games.map(g=>{const slug=row.key+'-'+slugify(g.n);return`
        <div class="gtile" data-slug="${slug}">
          <div class="gcover ${row.dealer?'dealer':''}" style="background:linear-gradient(160deg,${g.g[0]},${g.g[1]})">
            ...
            <button class="playbtn" aria-label="Play ${g.n}"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7-11-7Z"></path></svg></button>
          </div>

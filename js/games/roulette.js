...
<div class="rl-wrap">
  ...
  <div class="rl-bottom">
    ...
    /* racetrack neighbor preview */
    document.getElementById('rlRtEl').addEventListener('mouseover',e=>{
      document.querySelectorAll('#rlRtEl .rl-nb-preview,.rl-nb-center').forEach(el=>el.classList.remove('rl-nb-preview','rl-nb-center'));
      const t=e.target.closest('.rl-rt-n[data-n]');if(!t)return;
      const W=this.WHEEL,len=W.length,n=+t.dataset.n,idx=W.indexOf(n),count=this._nbCount;
      for(let d=-count;d<=count;d++){
        const el=document.querySelector(`#rlRtEl .rl-rt-n[data-n="${W[(idx+d+len)%len]}"]`);
        if(el)el.classList.add(d===0?'rl-nb-center':'rl-nb-preview');
      }
    });
    document.getElementById('rlRtEl').addEventListener('mouseleave',()=>{
      document.querySelectorAll('#rlRtEl .rl-nb-preview,.rl-nb-center').forEach(el=>el.classList.remove('rl-nb-preview','rl-nb-center'));
    });
...
.rl-top{display:flex;flex-direction:row;align-items:flex-start;justify-content:center;gap:10px}
...
/* streak */
.rl-streak{display:flex;flex-direction:column;gap:6px;padding:8px 7px;
  background:rgba(0,0,0,.22);border-radius:10px;box-sizing:border-box;width:46px;align-self:stretch}
.rl-streak-dots{display:flex;flex-direction:column;gap:5px;align-items:center;flex:1}
.rl-streak-empty{font-size:8px;color:rgba(255,255,255,.2);font-weight:600;letter-spacing:.02em;text-align:center;writing-mode:vertical-lr;transform:rotate(180deg);margin:auto}
.rl-streak-stats{display:flex;flex-direction:column;gap:4px;padding-top:6px;border-top:1px solid rgba(255,255,255,.06);align-items:center}
.rl-sstat{font-size:8px;font-weight:700;letter-spacing:.04em;display:flex;flex-direction:column;align-items:center;gap:1px}
.rl-sstat b{font-size:11px;font-weight:900}
...
.rl-streak-lbl{font-size:8px;font-weight:800;letter-spacing:.06em;color:#E6BE55;
  text-transform:uppercase;animation:sdotIn .22s ease both;text-align:center;line-height:1.3}
...
.rl-zero-col{flex-shrink:0;width:28px;align-self:stretch;display:flex}
.rl-zero{flex:1;min-height:106px;
  background:linear-gradient(158deg,#2E8A50,#10532C);
  box-shadow:inset 0 1px 0 rgba(255,255,255,.22),0 4px 14px rgba(0,0,0,.34);border-radius:6px!important}
.rl-mid-wrap{flex:1;display:flex;flex-direction:column;gap:4px}
.rl-num-and-col{display:flex;gap:4px}
.rl-num-area{
  flex:1;display:grid;
  grid-template-columns:repeat(11,minmax(0,1fr) 4px) minmax(0,1fr);
  grid-template-rows:repeat(2,32px 4px) 32px 3px 8px;
  position:relative}
.rl-col-bets{flex-shrink:0;width:28px;display:flex;flex-direction:column;gap:4px}
.rl-col-bets .rl-cell{flex:0 0 32px;height:32px}
.rl-doz-row,.rl-out-row{display:flex;gap:4px}
/* base cell */
.rl-cell{display:flex;align-items:center;justify-content:center;
  font-size:9px;font-weight:700;cursor:pointer;user-select:none;border-radius:5px;
...
.rl-col{background:rgba(255,255,255,.03);font-size:8px}
.rl-doz{flex:1;background:rgba(255,255,255,.03);height:22px;min-height:22px;font-size:10px}
.rl-doz sup{font-size:6px}
.rl-out{flex:1;background:rgba(255,255,255,.03);height:22px;min-height:22px;font-size:10px;letter-spacing:.02em}
...
.rl-rt-n{width:26px;height:26px;border-radius:5px;flex-shrink:0;
  ...
  font-size:9px;font-weight:700;cursor:pointer;user-select:none;position:relative;
